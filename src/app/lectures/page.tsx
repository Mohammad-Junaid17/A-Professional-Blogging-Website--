...
export default async function LecturesPage(props: { searchParams: Promise<{ category?: string; channel?: string; q?: string }> }) {
  ...
  const resultCount = lectures?.length ?? 0;
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <div className="flex items-center text-sm text-muted mb-6">
        ...
        {currentCategory || currentChannel ? (
          <>
            ...
            <ChevronRight size={14} className="mx-1" />
            <span className="text-foreground font-medium">{currentCategory || currentChannel}</span>
          </>
        ) : (
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          {(currentCategory || currentChannel) && (
            <p className="text-sm text-muted mt-0.5">{currentChannel ? "YouTube Channel Sync" : "Lectures"}</p>
          )}
        </div>
      ...
      <Suspense>
        <CategoryFilterBar
          ...
          allCategoryLabel="All Lectures"
          paramName="category"
          searchPlaceholder="Search lectures by keyword..."
          resultCount={resultCount}
          visibleCount={8}
          basePath="/lectures"
          section="lectures"
          extraGroup={CHANNELS.length > 0 ? { label: "Channels", paramName: "channel", items: CHANNELS } : undefined}
        />
      </Suspense>

      {!lectures || lectures.length === 0 ? (
        <div className="py-16 text-center">
          <p className="text-muted text-lg mb-2">No lectures found</p>
