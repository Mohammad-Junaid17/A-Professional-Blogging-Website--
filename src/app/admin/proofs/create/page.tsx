...
export default function CreateProof() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("proofs").insert(form);
  ...
  return (
    <div className="max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="bg-card border border-border rounded-xl p-6 space-y-6">
        ...
        <div><label className="block text-sm font-semibold text-foreground mb-1.5">Arabic Text</label><textarea value={form.arabic_text} onChange={(e) => set("arabic_text", e.target.value)} rows={4} dir="rtl" className="w-full p-3 bg-background border border-border rounded-lg text-foreground focus:outline-none focus:ring-2 focus:ring-primary font-amiri text-xl" /></div>
        ...
        <div><label className="block text-sm font-semibold text-foreground mb-1.5">English Translation *</label><textarea value={form.translation} onChange={(e) => set("translation", e.target.value)} rows={3} className="w-full p-3 bg-background border border-border rounded-lg text-foreground focus:outline-none focus:ring-2 focus:ring-primary" required /></div>
        <div className="flex justify-end pt-4 border-t border-border"><button type="submit" disabled={loading} className="flex items-center gap-2 bg-primary text-card px-8 py-3 rounded-lg font-bold hover:bg-primary/90 transition-colors disabled:opacity-50">{loading ? <Loader2 size={20} className="animate-spin" /> : <Save size={20} />} {loading ? "Saving..." : "Save Proof"}</button></div>
      </form>
    </div>
  );
}
