...
export default function CreateScholar() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("scholars").insert({ ...form, slug: form.slug || genSlug(form.name_english) });
    if (error) { setError(error.message); setLoading(false); } else { router.push("/admin/scholars"); router.refresh(); }
  };

  return (
    <div className="max-w-4xl">
      <div className="flex items-center gap-4 mb-8"><Link href="/admin/scholars" className="p-2 hover:bg-muted/10 rounded-lg"><ArrowLeft size={20} className="text-muted" /></Link><h1 className="text-3xl font-bold font-serif text-foreground">New Scholar</h1></div>
