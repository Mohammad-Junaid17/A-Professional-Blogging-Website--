...
export default function EditScholar({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    supabase.from("scholars").select("*").eq("id", id).single().then(({ data }) => {
      if (data) setForm({ name_english: data.name_english || "", name_arabic: data.name_arabic || "", slug: data.slug || "", birth_year: data.birth_year || "", death_year_ah: data.death_year_ah || "", madhab: data.madhab || "", origin: data.origin || "", bio: data.bio || "", status: data.status || "published", image_url: data.image_url || "" });
      setFetching(false);
    });
  }, [id]);
