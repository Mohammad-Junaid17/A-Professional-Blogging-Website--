...
export default function EditLecture({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ returnUrl?: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) { setError(error.message); setLoading(false); } else { router.push(returnUrl || "/admin/lectures"); router.refresh(); }
  };
  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-center gap-4 mb-8"><Link href={returnUrl || "/admin/lectures"} className="p-2 hover:bg-muted/10 rounded-lg"><ArrowLeft size={20} className="text-muted" /></Link><h1 className="text-3xl font-bold font-serif text-foreground">Edit Lecture</h1></div>
      ...
      <form onSubmit={handleSubmit} className="bg-card border border-border rounded-xl p-6 space-y-6">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-border">
          <DeleteButton id={id} table="lectures" redirectTo={returnUrl || "/admin/lectures"} />
          <button type="submit" disabled={loading} className="flex items-center gap-2 bg-primary text-card px-8 py-3 rounded-lg font-bold hover:bg-primary/90 transition-colors disabled:opacity-50">{loading ? <Loader2 size={20} className="animate-spin" /> : <Save size={20} />} {loading ? "Saving..." : "Update Lecture"}</button>
        </div>
      </form>
    </div>
  );
